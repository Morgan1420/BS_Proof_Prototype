"""Sequential, per-paper grade + conclusion-synthesis pipeline (Phase 5).
...
every ResearchPaper currently stored for that ingredient, in order:

  1. Grade it (app/services/paper_grader.py::grade_single_paper) — a
     no-op, no Gemini call, if it's already graded.
  2. If (and only if) its grade_score ends up > 50, synthesize/merge its
     findings into the ingredient's running PaperConclusion set
...
Each paper's two steps are individually wrapped in their own try/except
(see analyze_ingredient_papers below) — a failure grading or
synthesizing conclusions for one paper (rate limit, transient network
error, malformed Gemini response) is logged and skipped, never allowed
to abort the loop or roll back already-committed progress on earlier
...
def analyze_ingredient_papers(session: Session, ingredient_id: int) -> PipelineResult:
    """Runs the grade -> (maybe) synthesize-conclusions loop over every
    ResearchPaper currently stored for `ingredient_id`.
    ...
    an already-graded row), and conclusion merging is idempotent per
    (paper, conclusion) pair (a paper's id is only ever appended to a
    supporting/contradicting list once — see conclusion_grader.py).
